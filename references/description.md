...
- **An install instruction puts the checkout where the agent reads it.** Every channel a readme offers ends in the reader's own skills directory, whether an installer, a plugin manifest or a plain clone; a clone into some other directory is readable only through a symlink, which is one skill in two places and a path named for the author's machine rather than the reader's
- **A channel the readme offers is a channel the repository carries.** `/plugin marketplace add` reads `.claude-plugin/marketplace.json` out of the repository it is handed, so a readme that offers the command without shipping the file sends the reader to a command that refuses
