...
- **Review is never one's own.** A fresh separate agent reviews, never the author and never a fork, which inherits the assumptions the review must test; the house rules go into its prompt, it finds defects and edits nothing, and every finding is verified before it is shown. See [references/review.md](references/review.md)
